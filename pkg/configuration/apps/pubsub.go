...
func (p *PubSub) FetchConfig(b builder.Builder) error {
	config, err := b.Build()
	if err != nil {
		return fmt.Errorf("building pubsub conf, err: %w", err)
	}
	...
	p.Kafka.BrokerUrls = config.StringSlice("kafka.broker_urls")
